...
def test_get_li_message_targets(test_app, auth, campaign, session):
    ...
    campaign.create_janium_campaign_step(token, janium_campaign.janium_campaign_id, step_type_id=1, delay=1)
    campaign.create_janium_campaign_step(token, janium_campaign.janium_campaign_id, step_type_id=1, delay=3)
    assert session.query(Janium_campaign_step).filter(Janium_campaign_step.janium_campaign_id == janium_campaign.janium_campaign_id).count() == existing_janium_campaign_steps + 2
    ...
    ### Test First Step/li_message action ###
    # Create Connection Action #
    cnx_action = Action(str(uuid4()), new_contact.contact_id, 1, datetime.utcnow() - timedelta(days=4), None)
    session.add(cnx_action)
    session.commit()

    assert len(janium_campaign.get_li_message_targets()) == 1



    ### Test Continue Campaign Action ###
    # Create Response Action #
    response_action = Action(str(uuid4()), new_contact.contact_id, 2, datetime.utcnow() - timedelta(days=2), None)
    ...
    continue_action = Action(str(uuid4()), new_contact.contact_id, 14, (datetime.utcnow() - timedelta(days=2)) + timedelta(hours=1), None)
    ...
    message_action = Action(str(uuid4()), new_contact.contact_id, 3, datetime.utcnow() - timedelta(days=2), None)
