...
from foundation_api.V1.sa_db.model import Email_config, get_db_session
...
def poll_ulinc_webhook_task():
    ...
    with get_db_session() as session:
        ...
        if poll_and_save_webhook(ulinc_config, json_body['webhook_url'], json_body['webhook_type'], session) == "success":
...
def poll_ulinc_csv_task():
    ...
    with get_db_session() as session:
        json_body = request.get_json()
        ulinc_config = session.query(Ulinc_config).filter(Ulinc_config.ulinc_config_id == json_body['ulinc_config_id']).first()
        ulinc_campaign = session.query(Ulinc_campaign).filter(Ulinc_campaign.ulinc_campaign_id == json_body['ulinc_campaign_id']).first()
...
def refresh_ulinc_data_task():
    ...
    with get_db_session() as session:
        ...
        refresh_ulinc_cookie(ulinc_config, session)
        if ulinc_config.is_working:
            refresh_ulinc_account_status(ulinc_config, session)
...
def data_enrichment_task():
    ...
    with get_db_session() as session:
        json_body = request.get_json(force=True)
        contact = session.query(Contact).filter(Contact.contact_id == json_body['contact_id']).first()

        if data_enrichment_function(contact, session) == 'success':
            return jsonify({"message": "success"})
...
def send_email_task():
    ...
    with get_db_session() as session:
        ...
        if email_config := session.query(Email_config).filter(Email_config.email_config_id == json_body['email_target_details']['email_config_id']).first():
            if contact := session.query(Contact.contact_id == json_body['email_target_details']['contact_id']).first():
                if contact.is_messaging_task_valid():
                    send_email_function(email_config, json_body['email_target_details'], session)
                    return jsonify({"message": "success"})
...
def send_li_message_task():
    ...
    with get_db_session() as session:
        json_body = request.get_json(force=True)
        if contact := session.query(Contact.contact_id == json_body['li_message_target_details']['contact_id']).first():
            if contact.is_messaging_task_valid():
